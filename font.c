...
unsigned int *buf = (unsigned *)0x18410100;
unsigned char *bufc = (unsigned char*)0x18410100;

int uvl_entry()
{
	for (int i = 0; i < 0xE100; i++)
	{
		buf[i] = 0x00000000;
	}
	...
	k = (c-0x20)<<3;
		for(i = 0; i < 24; i+=3){
			c = font[k++];
			for(j = 0; j < 720*8; j+=90*8){
				offs = offs0+(239*3-i) + j;
				if(c & 0x80){
					color = 0xFF;
				}else{
					color = 0x00;
	...
	GSPGPU_FlushDataCache((void*)buf, 0x00038400);
	GX_SetTextureCopy((void*)buf, (void*)0x1F48F000, 0x00038400, 0, 0, 0, 0, 8);
	svcSleepThread(0x400000LL);
	GSPGPU_FlushDataCache((void*)buf, 0x00038400);
	GX_SetTextureCopy((void*)buf, (void*)0x1F4C7800, 0x00038400, 0, 0, 0, 0, 8);
	svcSleepThread(0x400000LL);
