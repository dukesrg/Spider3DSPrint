...
void print(char *value, unsigned fgcolor, unsigned bgcolor){
	unsigned pos = 0, offs, fontoffs, fboffs, x, y, color;
	unsigned char c;
	while((c = value[pos++]) != 0){
		if(col >= BOTTOM_COLS){
			col = 0;
		...
		switch(c){
			...
			case "\f":
				...
				for(fboffs = 0; fboffs < BOTTOM_FB_SIZE; fboffs += MEM_STRIPE_SIZE * BYTES_PER_PIXEL){
					CopyMem(BUFFER_LOC, (void*)(BOTTOM_FB0_LOC + fboffs), MEM_STRIPE_SIZE * BYTES_PER_PIXEL);
				}
				break;
			case "\r":
				col = 0;
			...
			default:
				if(c >= FONT_START){
					...
					fboffs = col * BOTTOM_Y_RES * CHAR_WIDTH * BYTES_PER_PIXEL;
					offs = ((BOTTOM_ROWS - row) * CHAR_HEIGHT - 1) * BYTES_PER_PIXEL;
					CopyMem(BOTTOM_FB0_LOC + fboffs, BUFFER_LOC, BOTTOM_Y_RES * CHAR_WIDTH * BYTES_PER_PIXEL);
					...
					CopyMem(BUFFER_LOC, (void*)(BOTTOM_FB0_LOC + fboffs), BOTTOM_Y_RES * CHAR_WIDTH * BYTES_PER_PIXEL);
					CopyMem(BUFFER_LOC, (void*)(BOTTOM_FB1_LOC + fboffs), BOTTOM_Y_RES * CHAR_WIDTH * BYTES_PER_PIXEL);
				}
				col++;
				break;
		}
